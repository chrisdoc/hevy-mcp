...
| Response contracts     | `packages/core/src/protocol/output-schemas.ts`, `packages/core/src/protocol/formatters.ts`, `packages/core/src/protocol/response-contracts.ts` | Focused modules own Zod output schemas, raw-to-public formatting, legacy text projection, and response assembly. |
...
| -------- | ------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------------------------------------- |
| Critical | Production output schemas and live canary schemas have separate ownership.      | Production schemas in `packages/core/src/protocol/output-schemas.ts`; projections and response assembly in `packages/core/src/protocol/formatters.ts` and `packages/core/src/protocol/response-contracts.ts`; test-local schemas in `tests/integration/hevy-mcp.integration.test.ts`; July 10 run and PR #594. | A live payload can satisfy a permissive canary assertion but fail SDK output validation, or tests can drift away from production. |
...
1. **Production response contracts are the oracle.** Export reusable,
   ...
   `packages/core/src/protocol/output-schemas.ts`. Raw-to-public normalization lives
   in `packages/core/src/protocol/formatters.ts`, while legacy text projection and
   response assembly live in `packages/core/src/protocol/response-contracts.ts`. Use
...
`packages/core/src/protocol/output-schemas.ts`,
`packages/core/src/protocol/formatters.ts`,

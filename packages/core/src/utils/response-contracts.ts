...
export const trainingSummaryWeekSchema = z.object({
	...
	exercise_count: z.number().int().nonnegative(),
	set_count: z.number().int().nonnegative(),
	working_set_count: z.number().int().nonnegative(),
});
...
export const trainingSummaryExerciseSessionSchema = z.object({
	...
	set_count: z.number().int().nonnegative(),
	working_set_count: z.number().int().nonnegative(),
	total_reps: z.number().nonnegative().nullable(),
	weighted_rep_volume_kg: z.number().nonnegative().nullable(),
	top_weight_kg: z.number().nullable(),
	top_reps: z.number().nonnegative().nullable(),
	top_rpe: z.number().nullable(),
	total_distance_meters: z.number().nonnegative().nullable(),
	total_duration_seconds: z.number().nonnegative().nullable(),
	total_custom_metric: z.number().nullable(),
});
...
export const trainingSummaryExerciseTrendCoverageSchema = z.object({
	eligible_exercise_count: z.number().int().nonnegative(),
	included_exercise_count: z.number().int().nonnegative(),
	exercise_limit: z.number().int().positive(),
	sessions_per_exercise_limit: z.number().int().positive(),
	truncated: z.boolean(),
});
...
const trainingSummaryOutputSchema = {
	...
	workouts: z.object({
		...
		exercise_trends: z.array(trainingSummaryExerciseTrendSchema),
		exercise_trend_coverage: trainingSummaryExerciseTrendCoverageSchema,
	}),

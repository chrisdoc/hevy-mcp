...
defaults and is bounded to 10 items (100 for exercise-template search). Exercise
search scans at most 10 API pages by default; use `--max-pages N` to change the
bound (up to 100).
...
stdout. Usage and validation errors exit 2; configuration errors exit 1; API failures exit 3,

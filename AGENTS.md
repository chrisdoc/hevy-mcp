...
- The codebase is TypeScript (Node.js v20+), with tool implementations in `src/tools/` and shared utilities in `src/utils/`.
- The Hevy API client comes from the `hevy-api-client` npm package and is wrapped by `src/utils/hevyApiClient.ts`.
...
### Working with the Hevy API client
- `src/utils/hevyApiClient.ts` wraps the `hevy-api-client` npm package.
- To pick up API changes, update the `hevy-api-client` dependency.
...
3. **Build failures:** Run `pnpm run check` to identify formatting/linting issues
4. **Type errors in tool handlers:** Use `InferToolParams<typeof schema>` instead of manual type assertions
5. **Linter warnings about `any`:** Expected in `webhooks.ts` where API methods don't exist yet

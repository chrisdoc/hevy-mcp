...
| `npm run test:worker-http:live` | Read-only local Wrangler Worker canary against the real Hevy API.                                     | Requires `HEVY_RUN_LIVE_WORKER_TESTS=1` and `HEVY_API_KEY`; trusted CI only.                         |
...
The Hevy API client, types, schemas, and mocks under `src/generated/` are
generated by Kubb. Never edit files in that directory manually.
...
`src/shared-server.ts` constructs the tools, prompts, resources, and native-fetch
Hevy client used by both runtimes:

- `src/cli.ts` and `src/index.ts` are Node-only. Keep process lifecycle, Node
  built-ins, stdio transport, telemetry, and stdio observability on this path.
- `src/worker.ts` is the Cloudflare Worker Streamable HTTP entry point. It must
  not import the Node-only path.
- Modules imported by `src/shared-server.ts` must remain safe in both Node.js
  and Cloudflare Workers.

`src/utils/stdio-observability.ts` instruments private MCP SDK stdio fields such
as `_ondata` and `_readBuffer`. After every `@modelcontextprotocol/sdk` upgrade,

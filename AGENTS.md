...
- **`npm run check:types`**: May report TypeScript errors in source code due to strict type checking. The project builds successfully via `npm run build` (using tsup) despite these errors. Generated code in `src/generated/` is excluded from type checking.

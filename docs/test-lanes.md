...
`mise exec -- pnpm --silent run report:test-lanes`; use
`mise exec -- pnpm --silent run report:test-lanes -- --format=json` for every registered
...
lanes. The silent pnpm invocation keeps its script banner off stdout, and the
inventory command sends build logs to stderr so redirected report output remains
valid Markdown or JSON.
...
| `contract`                 | pnpm run test:contract (Nx: repository:test:contract)                 | blocking      | node-24           | —                                                  | —                                                                 | vitest; include: packages/node/src/index.test.ts, packages/node/src/utils/config.test.ts, packages/core/src/protocol/register.test.ts, packages/core/src/protocol/output-schemas.test.ts, tests/unit/server-manifest.test.ts, tests/contract/runtime-contract-matrix.test.ts |
